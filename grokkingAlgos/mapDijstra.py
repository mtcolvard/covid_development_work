...
def filter_by_angle():
    ...
    # for key, value in filtered_waypoints.items():
    #     new_filtered_waypoints = {k: v.update({k:None}) for k,v in value.items() if v > (k['destination'] + math.pi/4)}
    new_filtered_waypoints = {key: value({k: v.update({k: 'None'}) for k,v in value.items() if v > (k['destination'] + math.pi/4)}) for key, value in filtered_waypoints.items()}

            # k = None if v > (k['destination'] + math.pi/4):
        # for k, v for value.items():
        #     k = None if v > (k['destination'] + math.pi/4):


    print(filtered_waypoints)
    print(new_filtered_waypoints)
