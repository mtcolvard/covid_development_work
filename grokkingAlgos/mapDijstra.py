...
def populate_graph_distances_angles():
    waypoints_lon_lat = {k:v['lon_lat'] for k, v in total_dict.items()}
    waypoint_ids = list(total_dict.keys())
    # waypoint_ids.pop()
    # unprocessed_waypoints = deque(list(total_dict.keys()))
    ...
    for waypoint_id in waypoint_ids:
        ...
        for each_id in waypoint_ids:
            filtered_waypoints[waypoint_id][each_id] = crowflys_bearing(waypoints_lon_lat[waypoint_id], waypoints_lon_lat[each_id])
    ...
    while waypoint_route_order:
        if next_park == 'destination':
            break
        angle_filtered_park_options = {k:v for k,v in filtered_waypoints[waypoint_route_order[-1]].items() if
        v[1] != 0 and
        v[1] < (filtered_waypoints[waypoint_route_order[-1]]['destination'][1] + math.pi/5) and
        v[1] > (filtered_waypoints[waypoint_route_order[-1]]['destination'][1] - math.pi/5)}
        ...
        print('next_park', next_park)
    print(waypoint_route_order)
    # return filtered_waypoints

# def select_next_park(filtered_waypoints):
#     # filtered_parks = {}
#     for waypoint, crowflys_bearings in filtered_waypoints.items():
#         # for key, value in crowflys_bearing.items():
#         # filtered_parks[waypoint] = {}
#         filtered_parks = {k:v for k,v in crowflys_bearings.items() if
#             v[1] < (crowflys_bearings['destination'][1] + math.pi/12) and
#             v[1] > (crowflys_bearings['destination'][1] - math.pi/12)}
#     # next_waypoint_key = min(filtered_parks[''], key=lambda v: parks_within_perp_distance[v]['size_in_hectares'])
#
#         print('filtered_parks', len(filtered_parks), filtered_parks)

        # next_park = {k:v for k,v in sorted(crowflys_bearing.items(), key=lambda item: item[1][1])}

            # lowest_distance = float('inf')
            # lowest_distance_node = None
            # waypoint_distance_angle_dict = filtered_waypoints[waypoint_route_order[-1]]
            # for node in waypoint_distance_angle_dict:
            #     distance = waypoint_distance_angle_dict[node][0]
            #     if distance < lowest_distance and node not in waypoints_processed:
            #         lowest_distance = distance
            #         lowest_distance_node = node
            # return lowest_distance_node

populate_graph_distances_angles()
